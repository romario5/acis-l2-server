...
public class AdminSkill implements IAdminCommandHandler
{
	...
	private static void showSkillList(Player player, Player targetPlayer, int page)
	{
		...
		final Map<Object, Optional<L2Skill>> data = SkillTable.getInstance().getSkills().stream().filter(s -> s.getLevel() < 99).collect(Collectors.groupingBy(s -> s.getId(), Collectors.maxBy(COMPARE_SKILLS_BY_LVL)));
