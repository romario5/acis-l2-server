...
public class AdminNpc implements IAdminCommandHandler
{
		...
		else if (command.startsWith("admin_walker"))
		{
			...
			if (st.hasMoreTokens())
			{
				...
				else
				{
					...
					{
						player.sendMessage("Usage: //walker <npc_id>");
					}
				}
			}
	...
	private static void showShopList(Player player, int listId)
	{
		...
		for (Product product : buyList.values())
	...
	private static void sendWalkerInfos(Player player)
	{
		...
		for (Entry<Integer, List<WalkerLocation>> entry : WalkerRouteData.getInstance().getWalkerRoutes().entrySet())
		{
			...
			StringUtil.append(sb, "<tr><td width=180>NpcId: ", entry.getKey(), " - Path size: ", entry.getValue().size(), "</td><td width=50><a action=\"bypass admin_teleport ", teleLoc, "\">Tele. To</a></td><td width=50 align=right><a action=\"bypass admin_walker ", entry.getKey(), "\">Show</a></td></tr>");
		}
