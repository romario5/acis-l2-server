...
public class AdminSpawn implements IAdminCommandHandler
{
	...
	public void useAdminCommand(String command, Player player)
	{
		if (command.startsWith("admin_list_spawns"))
		{
			...
			catch (Exception e)
			{
				...
				final WorldObject worldObject = player.getTarget();
				if (worldObject instanceof Npc)
					npcId = ((Npc) worldObject).getNpcId();
			}
		...
		else if (command.startsWith("admin_delete"))
		{
			...
			player.sendMessage("Deleted " + npc.getName() + " from " + npc.getObjectId() + ".");
		}
	}
	...
	private static void spawn(Player player, String monsterId, int respawnTime, boolean permanent)
	{
		WorldObject worldObject = player.getTarget();
		if (worldObject == null)
			worldObject = player;
		...
		try
		{
			...
			spawn.setLoc(worldObject.getX(), worldObject.getY(), worldObject.getZ(), player.getHeading());
