...
public class AdminFind implements IAdminCommandHandler
{
		...
		if (command.startsWith("admin_find"))
		{
			...
			switch (param)
			{
				...
				case "dualbox":
					...
					try
					{
						multibox = Integer.parseInt(nameIpOrPage);
						if (multibox < 1)
						{
	...
	private static void listPlayersPerName(Player player, String partOfName)
	{
		...
		for (Player targetPlayer : World.getInstance().getPlayers())
		{
			String name = targetPlayer.getName();
			if (name.toLowerCase().contains(partOfName.toLowerCase()))
			{
				charactersFound++;
				StringUtil.append(sb, "<tr><td><a action=\"bypass -h admin_debug ", name, "\">", name, "</a></td><td>", targetPlayer.getTemplate().getClassName(), "</td><td>", targetPlayer.getStatus().getLevel(), "</td></tr>");
			}
	...
	private static void listPlayersPerIp(Player player, String IpAdress) throws IllegalArgumentException
	{
		...
		for (Player targetPlayer : World.getInstance().getPlayers())
		{
			GameClient client = targetPlayer.getClient();
	...
	private static void listPlayersPerAccount(Player player, String name)
	{
		...
		html.replace("%characters%", String.join("<br1>", targetPlayer.getAccountChars().values()));
