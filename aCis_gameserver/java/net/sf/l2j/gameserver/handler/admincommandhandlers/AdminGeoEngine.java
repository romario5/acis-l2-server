				...
				switch (st.nextToken())
				{
					...
					case "pos":
						...
						if (block.hasGeoPos())
						{
							...
							// player.sendMessage("NSWE: " + block.getClass().getSimpleName());
					...
					case "move":
						...
						SpawnLocation tLoc = worldObject.getPosition();
			...
			try
			{
				switch (st.nextToken())
				{
					case "find":
						...
						final List<Location> path = GeoEngine.getInstance().findPath(player.getX(), player.getY(), player.getZ(), worldObject.getX(), worldObject.getY(), worldObject.getZ(), true, debug);
						if (path == null)
						{
