...
public class AdminItem implements IAdminCommandHandler
{
	...
	private static void createItem(Player player, Player targetPlayer, int id, int num, int radius)
	{
		...
		if (radius > 0)
		{
			final List<Player> players = player.getKnownTypeInRadius(Player.class, radius);
			for (Player knownPlayer : players)
				knownPlayer.addItem("Admin", id, num, player, true);
			
			player.sendMessage(players.size() + " players rewarded with " + num + " " + item.getName() + " in a " + radius + " radius.");
		}
		else
		{
