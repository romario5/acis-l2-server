...
public class Q242_PossessorOfAPreciousSoul extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("31748-03.htm"))
		{
			if (player.getInventory().hasItems(ORB_OF_BINDING))
			{
				...
				int cornerstones = st.getInteger("cornerstone");
				cornerstones++;
				if (cornerstones == 4)
				{
					st.unset("orb");
	...
	public String onKill(Npc npc, Creature killer)
	{
		...
		int orbs = st.getInteger("orb"); // check orbs internally, because player can use them before he gets them all
		if (orbs < 4)
		{
			orbs++;
