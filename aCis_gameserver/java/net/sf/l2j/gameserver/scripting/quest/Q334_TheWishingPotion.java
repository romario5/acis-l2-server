...
public class Q334_TheWishingPotion extends Quest
{
	...
	public String onKill(Npc npc, Creature killer)
	{
		...
		switch (npc.getNpcId())
		{
			...
			case AMBER_BASILISK:
				checkDropAndReward(player, st, cond, AMBER_SCALE);
				break;
			
			case ANT_SOLDIER:
			case ANT_WARRIOR_CAPTAIN:
				checkDropAndReward(player, st, cond, ANT_SOLDIER_APHID);
				break;
			
			case GLASS_JAGUAR:
				checkDropAndReward(player, st, cond, GLASS_EYE);
				break;
			
			case HORROR_MIST_RIPPER:
				checkDropAndReward(player, st, cond, HORROR_ECTOPLASM);
				break;
			
			case SILENOS:
				checkDropAndReward(player, st, cond, SILENOS_HORN);
				break;
			
			case TURAK_BUGBEAR:
			case TURAK_BUGBEAR_WARRIOR:
				checkDropAndReward(player, st, cond, BUGBEAR_BLOOD);
				break;
			
			case TYRANT:
			case TYRANT_KINGPIN:
				checkDropAndReward(player, st, cond, TYRANTS_CHITIN);
				break;
			
			case WHISPERING_WIND:
				checkDropAndReward(player, st, cond, WIND_SOULSTONE);
				break;
			
			case SUCCUBUS_OF_SEDUCTION:
				if (cond == 5 && !player.getInventory().hasItems(FORBIDDEN_LOVE_SCROLL) && st.getInteger("wish") == 1)
				{
			...
			case GRIMA:
				if (cond == 5 && st.getInteger("wish") == 2 && Rnd.get(1000) < 33)
				{
					st.set("wish", "0");
					playSound(player, SOUND_ITEMGET);
