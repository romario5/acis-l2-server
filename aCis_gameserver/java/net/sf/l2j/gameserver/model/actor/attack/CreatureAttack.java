...
public class CreatureAttack<T extends Creature>
{
	...
	private HitHolder[] doAttackHitByPole(Attack attack, Creature target, int sAtk)
	{
		...
		if (maxAttackedCount > 1)
		{
			...
			for (final Creature obj : _actor.getKnownTypeInRadius(Creature.class, _actor.getStatus().getPhysicalAttackRange()))
			{
				if (obj == target)
					continue;
				...
				if (!obj.isAttackableBy(_actor))
					continue;
				
				if (_actor instanceof Playable && obj instanceof Playable && obj.isAttackableWithoutForceBy((Playable) _actor))
				{
					if (obj.isInsideZone(ZoneId.PEACE) || !isMainTargetPlayable)
						continue;
				}
				...
				hitHolders.add(getHitHolder(attack, obj, false));
			}
		}
